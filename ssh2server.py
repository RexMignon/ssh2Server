...
connection_executor: concurrent.futures.ThreadPoolExecutor = concurrent.futures.ThreadPoolExecutor(max_workers=CONNECTION_POOL_SIZE)
...
class ReverseTunnelThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            ui_logger.info(f"反向隧道 '{self.comment}' 连接成功。现在访问 {self.server_info['host']}:{remote_port} 将转发到 {self.local_info[0]}:{self.local_info[1]}。")
            ...
            error_message = str(e)
            if "Authentication failed" in error_message: self._log('error', "认证失败，请检查用户名和密码。")
            elif "Could not resolve hostname" in error_message: self._log('error', "无法解析SSH主机名。")
            elif "Administratively prohibited" in error_message or "bind failed" in error_message: self._log('error', f"绑定远程端口失败。请检查：1. 远程端口 {self.remote_info[1]} 是否已被占用。 2. SSH服务器配置 /etc/ssh/sshd_config 中是否已设置 GatewayPorts yes。")
...
def attempt_connection(tunnel_id: str, server_group: ServerGroup, rule: ForwardRule, is_reconnect: bool = False):
    ...
    try:
        ...
        if rule.tunnel_type == 'reverse':
            server_info = {'host': conn.ssh_server_host, 'port': conn.ssh_server_port, 'user': conn.ssh_username, 'pass': conn.ssh_password}
            ...
            ui_logger.info(f"准备创建 [正向] 隧道 '{rule.comment}': 本地 {rule.local_host}:{rule.local_port} -> 远程 {rule.remote_host}:{rule.remote_port}")
        ...
        if isinstance(server, sshtunnel.SSHTunnelForwarder):
            ...
            server.start()
            ui_logger.info(f"正向隧道 '{rule.comment}' 连接成功。现在访问 {rule.local_host}:{rule.local_port} 将转发到 {rule.remote_host}:{rule.remote_port}。")
        else:
            server.start()

    except Exception as e:
        with tunnel_lock: active_tunnels.pop(tunnel_id, None)
        error_message = str(e)
        if "Authentication failed" in error_message: ui_logger.error(f"隧道 '{rule.comment}' 认证失败，请检查用户名和密码。")
        elif "Could not resolve hostname" in error_message: ui_logger.error(f"隧道 '{rule.comment}' 无法解析SSH主机名。")
        else: ui_logger.error(f"隧道 '{rule.comment}' 连接失败: {error_message}")
...
@socketio.on('connect')
def handle_connect():
    if not hasattr(g, 'is_connected'): print("前端 WebSocket 连接成功"); g.is_connected = True
@app.route('/')
...
def test_server_connection_with_paramiko() -> Union[Response, Tuple[Response, int]]:
    ...
    host, port, user, password = data.get('ssh_host'), int(data.get('ssh_port', 22)), data.get('ssh_user'), data.get('ssh_pass')
    if not all([host, user]): return jsonify({'status': 'error', 'message': '缺少主机和用户'}), 400
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        client.connect(hostname=host, port=port, username=user, password=password, timeout=10, look_for_keys=False, allow_agent=False)
        return jsonify({'status': 'success', 'message': 'SSH 连接成功!'})
    except socket.timeout: return jsonify({'status': 'error', 'message': f'连接超时: {host}:{port}'}), 400
    except paramiko.AuthenticationException: return jsonify({'status': 'error', 'message': '认证失败'}), 400
    except paramiko.SSHException as e: return jsonify({'status': 'error', 'message': f'SSH 错误: {e}'}), 400
    except Exception as e: return jsonify({'status': 'error', 'message': f'未知错误: {e}'}), 400
    finally:
        try: client.close()
...
def add_server() -> Response:
    data = request.json
    new_group = ServerGroup(id=generate_unique_id(), name=data['name'], ssh_connection=SSHConnectionConfig(ssh_server_host=data['ssh_host'], ssh_server_port=int(data['ssh_port']), ssh_username=data['ssh_user'], ssh_password=data['ssh_pass']), forwards=[], enabled=True)
...
def edit_server(server_id: str) -> Union[Response, Tuple[Response, int]]:
    ...
    conn.ssh_server_host, conn.ssh_server_port, conn.ssh_username = data['ssh_host'], int(data['ssh_port']), data['ssh_user']
...
def add_tunnel() -> Union[Response, Tuple[Response, int]]:
    ...
    new_rule = ForwardRule(id=generate_unique_id(), tunnel_type=tunnel_type, local_host=data['local_host'], local_port=int(data['local_port']), remote_host=data['remote_host'], remote_port=int(data['remote_port']), comment=data['comment'], enabled=True)
...
def edit_tunnel(server_id: str, rule_id: str) -> Union[Response, Tuple[Response, int]]:
    ...
    if new_tunnel_type not in ['forward', 'reverse']: return jsonify({'status': 'error', 'message': '无效的隧道类型'}), 400
    needs_restart = (rule.local_host != data['local_host'] or rule.local_port != int(data['local_port']) or rule.remote_host != data['remote_host'] or rule.remote_port != int(data['remote_port']) or rule.tunnel_type != new_tunnel_type)
    rule.tunnel_type, rule.local_host, rule.local_port, rule.remote_host, rule.remote_port, rule.comment = new_tunnel_type, data['local_host'], int(data['local_port']), data['remote_host'], int(data['remote_port']), data['comment']
...
def main():
    ...
    except Exception: print("无法自动打开浏览器，请手动访问上面的地址。")
